...
const { getArchivedWishLists, getWishList, getWishLists } = require('./get');
...
router
  .route('/wish-lists/:wishListId')
  .get([authenticateJwt, getWishList])
  .patch([authenticateJwt, updateWishList])
  .delete([authenticateJwt, deleteWishList]);

router.route('/users/:userId/wish-lists').get([authenticateJwt, getWishLists]);

router
  .route('/users/:userId/wish-lists/archived')
